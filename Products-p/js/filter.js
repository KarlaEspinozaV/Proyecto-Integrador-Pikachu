...
boxPrice.forEach( box => {
  box.addEventListener('click', (e) => {
    ...
    if (!priceRange1.checked && !priceRange2.checked && !priceRange3.checked && !priceRange4.checked) {
    ...
    if (priceRange1.checked) {
      items.filter((item) => item.price <= 100).forEach(
        item => filteredPrices.push(item)
      );

    } if (priceRange2.checked) {
      items.filter((item) => item.price >= 101 && item.price <= 250 ).forEach(
        item => filteredPrices.push(item));

    } if(priceRange3.checked) {
      filteredPrices.push(... items.filter((item) => item.price >= 251 && item.price <= 500 ));
